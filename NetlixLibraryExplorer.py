...
def filter_recent_content(df):
    ...
    recent_content = df[df['release_year']>2015][['title','type','release_year']].head()
...
def sort_by_release_year(df):
    ...
    sorted_df = df.sort_values(by='release_year', ascending=False)[['title','type','release_year']].head()
